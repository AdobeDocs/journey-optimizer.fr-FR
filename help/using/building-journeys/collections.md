...
# Cas pratique : transmission dynamique de collections à l’aide d’actions personnalisées{#passing-collection}
...
1. Dans le **[!UICONTROL Action parameters]** , collez l’exemple JSON. La structure affichée est statique : lorsque vous collez la payload, tous les champs sont définis comme des constantes.
...
1. Pour chaque champ, définissez le libellé qui s’affichera dans la zone de travail du parcours.
...
1. Dans le **[!UICONTROL Action parameters]** , définissez le paramètre de tableau (&quot;products&quot; dans notre exemple) à l’aide de l’éditeur d’expression avancé.
...
Pour le champ de tableau, vous pouvez également utiliser l’éditeur d’expression avancé pour effectuer une manipulation de données. Dans l’exemple suivant, nous utilisons la méthode [filter](functions/functionfilter.md) et [intersection](functions/functionintersect.md) fonctions :

...
Les **campagnes** se divisent en trois types :

* **Campagnes d’action** : communications ponctuelles ou récurrentes envoyées à une audience spécifique, idéales pour les messages autonomes tels que les annonces promotionnelles ou les newsletters.
* **Campagnes déclenchées par API** : campagnes déclenchées par le biais d’appels d’API, ce qui permet l’intégration à des systèmes externes pour envoyer des messages en fonction des événements en temps réel ou de la logique commerciale.
* **Campagnes orchestrées** : campagnes à plusieurs étapes basées sur l’audience, créées sur une zone de travail qui peut inclure des conditions, des temps d’attente et plusieurs actions pour créer des expériences planifiées et coordonnées.

**Bonne pratique** : utilisez des parcours pour un engagement complexe déclenché par un événement avec une orchestration avancée ; des campagnes d’action pour des communications planifiées basées sur l’audience ; des campagnes déclenchées par API pour le déclenchement programmatique à partir de systèmes externes ; et des campagnes orchestrées pour des communications à plusieurs étapes avec des exigences spécifiques à la campagne.
...
* La politique de fusion affecte les attributs et les identités accessibles dans des conditions de parcours, la personnalisation et les actions

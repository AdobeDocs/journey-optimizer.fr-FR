...
>[!NOTE]
>
>Vous ne pouvez pas transmettre une collection simple dans des paramètres d’action personnalisés. Les champs de collection plus complexes (tableaux d’objets) ne sont pas pris en charge.  Notez également qu’un format spécifique est attendu pour les paramètres (par exemple : chaîne, décimal, etc.). Vous devez veiller au respect de cette exigence.

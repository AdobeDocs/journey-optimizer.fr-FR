...
* **`inAudience()`de condition de lecture :** lors de l’utilisation de `inAudience()` dans un nœud de condition dans un parcours Lecture d’audience, l’appartenance à un segment est lue à partir de la projection par lots du profil. L’actualisation des données dans cette projection entraîne une TSL allant jusqu’à **2 heures** après l’ingestion. Pour plus d&#39;informations sur les scénarios de durée de propagation, consultez la documentation de la fonction [inAudience](functions/functioninaudience.md#propagation-timing).

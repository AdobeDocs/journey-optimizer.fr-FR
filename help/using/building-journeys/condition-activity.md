...
**Sur cette page :** découvrez comment utiliser l’activité Condition pour acheminer les profils vers différents chemins de parcours en fonction des règles, des données et de l’appartenance à une audience.
...
* **TL;DR:** Cette page décrit l’activité de condition dans Journey Optimizer, couvrant les cinq types de condition disponibles (Source de données, Heure, Partage en pourcentage, Date et Limite de profil) et comment acheminer les profils vers différents chemins de parcours en fonction des règles, des données ou de l’appartenance à une audience.

**Intentions:**
* Ajoutez une activité Condition à un parcours et créez plusieurs chemins d’embranchement
...
* Utiliser une répartition en pourcentage pour distribuer de manière aléatoire les profils sur plusieurs chemins d’accès
* Application d’une limitation de profil pour limiter le nombre de profils prenant un chemin de parcours spécifique
* Utilisation d’une vérification de l’appartenance à une audience comme condition dans un chemin de parcours

**Glossaire:**
* **Activité de condition** : activité de parcours qui évalue les règles et achemine les profils vers différents chemins d’accès en fonction des *de résultats (spécifiques au produit)*
* **Condition de Source de données** : type de condition qui évalue les champs des sources de données ou des événements de parcours à l’aide de l’éditeur d’expression *(spécifique au produit)*
* **Condition de temps** : type de condition qui filtre les profils en fonction de l’heure, du jour de la semaine ou d’une combinaison des deux *(spécifique au produit)*
* **Partage en pourcentage** : type de condition qui répartit de manière aléatoire les profils sur plusieurs chemins à l’aide d’un mécanisme statistique Java aléatoire *(spécifique au produit)*
* **Limite de profils** : type de condition qui limite le nombre de profils pouvant prendre un chemin spécifique. Les profils supplémentaires sont acheminés vers un autre chemin *(spécifique au produit)*
* **Chemin alternatif** : un chemin de secours activé lorsqu&#39;une erreur, un délai d&#39;expiration ou une limite de profil est atteinte *(spécifique au produit)*
...
* Les champs de schéma sans données ingérées sont interprétés comme nuls ; isEmpty() et isNull() donnent la valeur true pour ces champs, ce qui peut provoquer un comportement inattendu
...
* La limite de profils par défaut est de 1 000 ; le compteur se réinitialise lorsque le parcours est dupliqué ou qu’une nouvelle version est créée, mais pas entre les récurrences d’un parcours récurrent
* Pour une coiffe supérieure à 10 000, injectez au moins 1,3 fois le nombre de profils de la coiffe ; pour une coiffe inférieure à 10 000, injectez au moins 1 000 plus la coiffe
* La limite de profils n’est pas appliquée en mode test.
* Les requêtes de série temporelle (par exemple, la liste des achats ou les clics précédents) ne sont pas prises en charge dans l’éditeur d’expression simple. L’éditeur avancé doit être utilisé

**Terminologie:**
* Nom canonique : Activité de condition — Acronyme : none — variantes : nœud de condition, étape de condition
* Synonymes : « Condition de Source de données » = « condition basée sur l’expression » ; « Partage en pourcentage » = « partage aléatoire »
* Ne les confondez pas : « Partage en pourcentage » ≠ « Limite de profil » (le partage en pourcentage distribue aléatoirement tous les profils ; la limite de profil arrête le routage vers un chemin une fois qu’un seuil de nombre est atteint)

**FAQ:**
* **Q : Que se passe-t-il lorsque plusieurs chemins sont définis et qu’un profil répond à plusieurs conditions ?** — Seul le premier chemin éligible (de haut en bas sur la zone de travail) est exécuté ; l’ordre des chemins détermine la priorité.
* **Q : Puis-je ajouter un chemin de secours pour les profils qui ne correspondent à aucune condition ?** — Oui, activer l’option « Afficher le chemin pour d’autres cas que celui ou ceux ci-dessus » — sauf dans les conditions Partage en pourcentage, où tous les profils entrent toujours dans l’un des chemins de partage.
* **Q : Pourquoi ma condition isEmpty() est-elle évaluée sur true pour un champ dont je m’attends à obtenir les données ?** — Si le champ de schéma existe mais n’a pas été ingéré pour lui, Journey Optimizer et le profil client en temps réel l’interprètent comme une valeur nulle, de sorte que isEmpty() et isNull() renvoient la valeur true.
...
* **Q : Comment fonctionne la répartition en pourcentage en mode test ?** — En mode test, la branche supérieure est toujours choisie quels que soient les pourcentages de partage configurés.

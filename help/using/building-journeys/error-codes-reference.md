...
| **CJMPTS-1006-404** | Échec de la notification push/SMS avec « ressource introuvable » | Le fournisseur/canal référencé n’existe pas, est mal orthographié ou a été supprimé. | &#x200B;1. Vérifiez et corrigez les références et les identifiants de fournisseur/canal <br/>2. Vérifiez la configuration des sandbox/organisations.<br/>3. Vérifiez que les configurations de canal sont actives.<br/>4. Recréez la configuration du canal si nécessaire.<br/><br/>**Documentation associée** : [Surfaces de canal](../configuration/channel-surfaces.md) |
...
| **CJMMAS-1603-403** | Action interdite lors de la mise à jour/publication d’un modèle ou d’un préréglage | L’utilisateur ou l’utilisatrice ne dispose pas des autorisations/rôles requis ou l’action n’est pas autorisée dans l’état actuel. | &#x200B;1. Vérifiez que l’utilisateur dispose des autorisations appropriées (Gestionnaire des messages, Auteur, etc.)<br/>2. Vérifiez le statut du préréglage ou du modèle (brouillon, publié, archivé).<br/>3. Demandez l’accès à l’administrateur ou administratrice si nécessaire.<br/>4. Vérifiez les affectations de profils de produit.<br/><br/>**Documentation associée** : [Autorisations](../administration/permissions.md), [contrôle d’accès](../administration/permissions-overview.md) |
...
| Code d’erreur | Description | Cause principale | Résolution |
|------------|-------------|-----------|-----------|
| **CJMRPS-1047-409** | « Conflit. Le jeu de données a déjà été ajouté » lors de l’ajout d’un jeu de données de rapport. | Tentative d’ajout d’un jeu de données déjà configuré | &#x200B;1. Examinez la configuration du jeu de données dans les paramètres de création de rapports<br/>2. N’ajoutez pas à nouveau les jeux de données déjà présents.<br/>3. Utilisez les listes de contrôle de migration officielles pour les rapports de migration.<br/>4. Supprimez les références de jeux de données en double.<br/><br/>**Documentation associée** : [Vue d’ensemble des rapports](../reports/gs-reports.md), [rapports de campagne](../reports/campaign-global-report-cja.md), [rapports de parcours](../reports/journey-global-report-cja.md). |
...
7. **Escalader si nécessaire** : si l’erreur persiste après les étapes de résolution suivantes, [contactez l’assistance Adobe](../start/user-interface.md#support-ticket-guidelines) avec le code d’erreur complet, l’identifiant de la requête (le cas échéant), les étapes à reproduire et les détails de configuration pertinents.

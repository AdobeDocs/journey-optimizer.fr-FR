...
**Sur cette page :** découvrez comment utiliser des conditions dans l’activité Optimiser pour créer plusieurs chemins de parcours en fonction des sources de données, de l’heure, des dates, des divisions en pourcentage, des limites de profil ou de l’appartenance à une audience.
...
>[!NOTE]
>
>Le nouveau moyen de créer des chemins conditionnels dans les parcours est l’activité [Optimiser](optimize.md). Elle remplace l’ancienne activité **Condition**, qui a été supprimée de l’interface d’utilisation. Toute la logique conditionnelle est désormais gérée à l’aide des conditions de l’activité Optimiser présentées sur cette page.
>
...
* **TL;DR:** Cette page explique comment configurer des conditions dans l’activité Optimiser de Journey Optimizer, couvrant cinq types de conditions (Source de données, Heure, Partage en pourcentage, Date et Limite de profil) qui acheminent les profils vers différents chemins de parcours en fonction des règles, de l’heure ou de l’appartenance à une audience.

**Intentions:**
* Ajoutez une condition à un parcours à l’aide de l’activité Optimiser et sélectionnez une méthode de condition
* Créez plusieurs chemins d’embranchement et gérez leur ordre de priorité dans la zone de travail du parcours
* Configurer une condition de Source de données à l’aide de l’éditeur d’expression pour évaluer les attributs de profil ou d’événement
* Configurez une condition de temps pour acheminer les profils en fonction de l’heure ou du jour de la semaine
* Appliquez une Limitation de profil pour limiter le nombre de profils acheminés sur un chemin spécifique
* Utilisation d’une vérification de l’appartenance à une audience comme condition dans un chemin de parcours

**Glossaire:**
* **Activité Optimiser** : activité de parcours en cours qui remplace l’ancienne activité Condition ; toute logique de branchement conditionnel est désormais configurée via sa *déroulante Méthode (spécifique au produit)*
* **Condition de source de données** : méthode de condition qui évalue les champs des sources de données ou des événements de parcours à l’aide de l’éditeur d’expression *(spécifique au produit)*
* **Partage en pourcentage** : méthode de condition qui répartit de manière aléatoire les profils sur plusieurs chemins à l’aide d’un mécanisme aléatoire Java statistique *(spécifique au produit)*
* **Limite de profils** : méthode de condition qui achemine les profils vers un chemin alternatif une fois qu’un nombre maximal défini est atteint sur le chemin nominal *(spécifique au produit)*
* **Chemin nominal** : chemin principal du parcours associé à une condition de limitation de profil ; il a toujours la priorité sur le chemin secondaire *(spécifique au produit)*

**Mécanismes de sécurisation :**
* L’évaluation de la condition échoue pour les profils comportant plus de deux identités inter-appareils dans la banque de profils
* Les champs de schéma sans données ingérées sont interprétés comme nuls ; isEmpty() et isNull() donnent la valeur true pour ces champs
* Le fuseau horaire est défini au niveau du parcours, et non au niveau de la condition individuelle
* L’option « Afficher le chemin pour d’autres cas » n’est pas disponible dans les conditions Partage en pourcentage .
* La limite par défaut du profil est de 1 000 ; le compteur se réinitialise lors de la duplication du parcours ou de la création d’une nouvelle version, mais pas entre les récurrences
* Pour les capuchons supérieurs à 10 000, injectez au moins 1,3 fois le capuchon; pour les capuchons inférieurs à 10 000, injectez au moins 1 000 fois plus le capuchon
* La limite de profil n’est pas appliquée en mode test. En mode test, la branche supérieure est toujours choisie pour la Division en pourcentage

**Terminologie:**
* Nom canonique : Conditions — Acronyme : none — variantes : activité de condition, méthode de condition, embranchement conditionnel
* Synonymes : « Optimiser l’activité (méthode Condition) » = « ancienne activité Condition »
* Ne les confondez pas : « Partage en pourcentage » ≠ « Limite de profil » (le partage en pourcentage répartit tous les profils de manière statistique ; la limite de profil arrête le routage sur le chemin nominal après un seuil de comptage)

**FAQ:**
* **Q : L’activité Condition a disparu de mon interface utilisateur - Qu’est-ce qui l’a remplacée ?** — L&#39;activité Condition a été remplacée par l&#39;activité Optimiser. Sélectionnez « Condition » dans le menu déroulant Méthode pour obtenir le même comportement. Les parcours existants avec des activités Condition continuent à fonctionner et s’affichent désormais avec une icône Optimiser .
* **Q : Lorsque plusieurs chemins d’accès sont éligibles pour un profil, quel chemin d’accès est emprunté ?** — Seul le premier chemin éligible (le plus élevé sur la zone de travail) est exécuté. Vous pouvez modifier les priorités en réorganisant les chemins verticalement.
* **Q : Pourquoi ma condition isEmpty() renvoie-t-elle inopinément la valeur true ?** — Si le champ de schéma existe mais n&#39;a ingéré aucune donnée pour lui, Journey Optimizer l&#39;interprète comme null, ce qui fait que isEmpty() et isNull() renvoient true.
* **Q : Le compteur de limites de profil est-il réinitialisé sur un parcours récurrent ?** — Non, le compteur ne se réinitialise pas entre les récurrences ; il se réinitialise uniquement lorsque le parcours est dupliqué ou qu&#39;une nouvelle version est créée.
* **Q : Puis-je utiliser une audience Adobe Experience Platform comme condition ?** — Oui, déposez une activité Optimiser , sélectionnez « Condition de source de données », ajoutez un chemin d’accès et faites glisser l’audience depuis le nœud Audiences dans l’éditeur d’expression.

...
**Sur cette page :** découvrez comment utiliser et configurer les événements de qualification d’audience pour déclencher une entrée ou une progression de parcours lorsque des profils remplissent les critères d’une audience Adobe Experience Platform ou en sortent.
...
1. Cliquez dans le champ **[!UICONTROL Audience]** et sélectionnez les audiences à exploiter.
   ...
   >[!TIP]
   >
   >Pour identifier la méthode d’évaluation d’une audience avant de l’utiliser, ouvrez le menu **[!UICONTROL Audiences]**, sélectionnez l’audience et vérifiez le champ **[!UICONTROL Méthode d’évaluation]** : **Diffusion en continu**, **Lot** ou **Edge**. Vous pouvez également ajouter la colonne **[!UICONTROL Méthode d’évaluation]** à la liste des audiences dans cette activité. La méthode d’évaluation affecte le délai d’entrée et les bonnes pratiques à appliquer. Voir [Audiences par lots](#batch-speed-segment-qualification) et [Audiences en flux continu](#streamed-speed-segment-qualification).
...
>[!NOTE]
>
>Lorsqu’un profil est qualifié pour un segment de diffusion en continu dans Edge, cette appartenance est projetée d’Edge vers Hub avant que le parcours puisse agir dessus. Cette propagation d’Edge vers Hub prend généralement **15 à 30 minutes**. Si les profils ne rejoignent pas le parcours de qualification d’audience comme prévu, laissez cette fenêtre de propagation (en ajoutant une activité d’attente, le cas échéant) avant d’approfondir l’enquête. Pour les cas d’utilisation nécessitant une entrée en temps réel réelle, envisagez plutôt un déclencheur d’événement unitaire .

...
* Les pages **Modèle d’optimisation personnalisé** ont été mises à jour pour refléter les dernières améliorations du modèle, couvrant le fonctionnement du modèle d’ensemble, les exigences du jeu de données, les cas d’utilisation, les hypothèses clés et le comportement de démarrage à froid. Pour en savoir plus, consultez les sections [Experience Decisioning](../experience-decisioning/ranking/personalized-optimization-model.md) et [Offer Decisioning](../offers/ranking/personalized-optimization-model.md).
...
* La page **Vérifier et envoyer un message de publipostage direct** a été mise à jour afin de clarifier le délai d’exportation du publipostage direct et le comportement par lots, y compris le planning fixe d’exportation UTC de 4 heures, les raisons pour lesquelles plusieurs fichiers peuvent être générés en une seule journée, lorsque l’**[!UICONTROL Mettre à jour le profil]** s’exécute dans les parcours et des recommandations pour les scénarios d’un fichier par jour. [En savoir plus](../direct-mail/test-send-direct-mail.md#dm-export-timing)
...
* Une nouvelle section **Profils accessibles et utilisation de licence** a été ajoutée à la page **Prise en main des profils** en tant que source unique de vérité pour ce concept. Des références ciblées ont été ajoutées dans les sections Audiences, Campagnes et Prise de décision. [En savoir plus](../audience/get-started-profiles.md#engageable-profiles)

* La documentation sur les activités **Partage** a été mise à jour afin de documenter le champ **[!UICONTROL Code de segment]** disponible dans chaque paramètre de sous-ensemble, ce qui vous permet d’attribuer un identifiant unique à chaque segment d’audience à des fins de suivi et de création de rapports. [En savoir plus](../orchestrated/activities/split.md)

* La page **Configurer une dimension de ciblage** a été mise à jour afin de documenter les deux types de dimensions de ciblage disponibles dans les campagnes orchestrées : la dimension de ciblage intégrée **dimension de ciblage de profil** (aucune configuration requise) et **dimensions de ciblage personnalisées** basée sur des schémas relationnels. [En savoir plus](../orchestrated/target-dimension.md)
...
* L’ancienne page [Modèles d’optimisation automatique](../offers/ranking/auto-optimization-model.md) de la **gestion des décisions** a été réécrite afin de s’aligner sur la documentation mise à jour sur la prise de décision, y compris la vue d’ensemble de l’apprentissage par renforcement, les exigences et les limites, l’équilibre entre optimisation et apprentissage et les détails sur l’échantillonnage de Thompson. [En savoir plus](../offers/ranking/auto-optimization-model.md)
...
* La documentation **Copier des objets Journey Optimizer entre sandbox** comprend désormais une remarque importante pour les **campagnes orchestrées** : après l’import, dupliquez la campagne dans le sandbox cible et utilisez le doublon pour l’exécution afin de vous assurer que le reporting capture correctement les commentaires et les données de tracking. [En savoir plus](../configuration/copy-objects-to-sandbox.md#copy-to-sandbox)

* La page **Terminologie clé** a été remaniée : six nouveaux termes ont été ajoutés, une nouvelle section **Termes relatifs au conflit et à la hiérarchisation** a été ajoutée et un nouveau guide de désambiguïsation **Lorsque les termes se ressemblent** a été ajouté pour quatre paires de termes qui prêtent souvent à confusion. Les termes spécifiques à Adobe Experience Platform ont été supprimés et remplacés par une note renvoyant au glossaire Adobe Experience Platform. [En savoir plus](../start/terminology.md)

* La documentation sur les **liens profonds** a été complétée par une nouvelle section **Créer des liens profonds** présentant les deux options disponibles pour les e-mails (interface d’utilisation du Concepteur d’e-mail et code de l’éditeur de personnalisation) et la syntaxe de la fonction URL pour les SMS. La page **Créer un SMS** comprend désormais une étape de lien profond dans le flux de création de contenu. [En savoir plus](../email/deeplinks.md)

* La référence d’assistance **URL** a été mise à jour avec une section dédiée dans la documentation de personnalisation. [En savoir plus](../personalization/functions/helpers.md#url)

* Une limitation a été ajoutée à la documentation d’assistance **Métadonnées d’exécution** : la fonction n’est pas prise en charge dans les canaux entrants (web, expérience basée sur du code, message in-app, cartes de contenu). [En savoir plus](../personalization/functions/helpers.md#execution-metadata)

* Une nouvelle page **Recettes de personnalisation** a été ajoutée, fournissant des modèles de personnalisation prêts à l’emploi pour les cas d’utilisation les plus courants dans Journey Optimizer. Elle couvre les recettes de date et d’heure (mise en forme de date actuelle, compte à rebours jusqu’à l’expiration, calculs de jours restants, affichage de l’heure uniquement et détection des week-ends et des jours de la semaine), les recettes de chaîne (à l’aide de `replaceAll` avec affectation de variable) et les recettes de secours conditionnelles (secours de champs vides à l’aide de `isEmpty`). [En savoir plus](../personalization/personalization-recipes.md)

* La documentation **Syntaxe de personnalisation** a été mise à jour avec une introduction étendue clarifiant la différence entre les syntaxes Handlebars (`{{...}}`) et PQL (`{%= ... %}`), y compris un tableau d’utilisation, des conseils sur l’échappement de guillemets doubles littéraux et une nouvelle section **Règle de syntaxe PQL pour les clés d’attribut spéciales** couvrant les mots-clés réservés, les clés d’attribut avec tiret et les identifiants d’événement numériques. La note sur l’échappement des guillemets inversés a également été corrigée : les noms de champ avec tiret peuvent être référencés directement dans les blocs `{{...}}`. Seule la syntaxe des guillemets inversés ne fonctionne pas dans ce cas. [En savoir plus](../personalization/personalization-syntax.md)

* La documentation **Fonctions de date et d’heure** a été enrichie de nouveaux exemples réels : un modèle de compte à rebours pour `dateDiff`, un modèle conditionnel de week-end et de jour de semaine pour `dayOfWeek` (avec une note sur l’utilisation de l’activité Condition de parcours pour les cas d’utilisation de routage), et un modèle d’affichage de l’heure uniquement combinant `extractHours` et `extractMinutes` avec une gestion des zéros non significatifs. [En savoir plus](../personalization/functions/dates.md)

* La documentation **Fonctions de chaîne** a été mise à jour avec un nouvel exemple pour `replaceAll`, qui montre comment affecter le résultat à une variable `{% let %}` pour le réutiliser sur plusieurs expressions dans le même modèle. [En savoir plus](../personalization/functions/string.md#replace-all)

* La documentation **Fonctions de tableau** a été mise à jour avec une nouvelle section **Itérer sur un tableau** documentant l’assistant de bloc Handlebars `{{#each}}`, y compris une note expliquant que le `{{#each}}` est pris en charge dans l’éditeur de personnalisation uniquement et ne peut pas être utilisé dans les activités de condition de parcours. [En savoir plus](../personalization/functions/arrays-list.md#each-loop)

* La page **Commencer avec les jeux de données** a été mise à jour avec une nouvelle entrée **Entrant** dans la section Jeux de données système, documentant le _Jeu de données d’événement d’activité entrant AJO_. Une note a été ajoutée pour clarifier le fait qu’un profil doit recevoir au moins un message envoyé par [!DNL Journey Optimizer] avant que les messages entrants ne soient capturés dans ce jeu de données. [En savoir plus](../data/get-started-datasets.md#system-datasets)

* La documentation **Exporter le contenu du message** a été étoffée avec des **questions fréquentes sur l’export de messages** (contenu personnalisé, images et médias, liens suivis, informations d’identification personnelles, rétention, cas d’utilisation, etc.) et des **exemples de fichiers JSON exportés** pour les SMS et les e-mails. [En savoir plus](../configuration/message-export.md)

* Une nouvelle page **Schéma d’export de messages AJO** documente chaque champ du jeu de données d’export de messages AJO, avec les types de données et la hiérarchie de la payload d’e-mail et de SMS exportés. [En savoir plus](../configuration/message-export-schema.md)

* Une nouvelle page **Personnaliser les URL dans les e-mails** a été ajoutée. Elle consolide les conseils sur la personnalisation dynamique des URL, la personnalisation complète/de base des URL, la personnalisation des paramètres de tracking d’URL et les principaux mécanismes de sécurisation. [En savoir plus](../email/url-personalization.md)

* Une nouvelle section **Requêtes relatives aux règles métier** a été ajoutée à la page des exemples de requête. Elle fournit une requête de lac de données permettant de vérifier tous les abandons de profil en raison d’exclusions liées au capping de fréquence de parcours sur un parcours spécifique après une date spécifique. La requête inclut le champ `eventCodeReason` pour identifier si les profils ont été exclus, car une limite a été atteinte (`CAP_REACHED`) ou en raison d’une priorité inférieure (`LOWER_PRIORITY`). [En savoir plus](../reports/query-examples.md#business-rules-queries)

* La documentation des **Propriétés de parcours** a été mise à jour afin de documenter le nouvel indicateur **Taille actuelle de la payload du parcours** dans le panneau des propriétés de parcours. Ce champ en lecture seule indique la taille actuelle de la payload du parcours par rapport à la limite configurée (par exemple, 1,5 Mo sur 2 Mo). Cela vous permet de surveiller la complexité du parcours avant la publication et d’éviter les erreurs de publication liées à la taille. [En savoir plus](../building-journeys/journey-properties.md#journey-payload-size)

## Avril 2026 {#april-2026}

* Mise à jour de la documentation sur l’activité **Changement de dimension** afin de clarifier le fait que, tandis que l’activité utilise une jointure externe et conserve tous les enregistrements à l’étape de changement de dimension, les enregistrements sans profil correspondant dans la nouvelle dimension de ciblage sont exclus de manière implicite au moment de la diffusion du message. [En savoir plus](../orchestrated/activities/change-dimension.md)

* Amélioration des mécanismes de sécurisation de la documentation **Ajout d’un champ Cc aux e-mails**. Ils indiquent désormais que les adresses en Cc ne font pas l’objet d’une vérification du consentement ni des listes d’exclusion. Ils indiquent également que les ouvertures et les clics des e-mails envoyés à l’adresse en Cc sont pris en compte dans le nombre total d’ouvertures et de clics lors de l’analyse de l’envoi. [En savoir plus](../configuration/cc-email-field.md)

* Mise à jour de la documentation sur les **activités de canal** avec une nouvelle section **Messages marketing ou transactionnels** expliquant les différences de comportement entre les deux catégories de canal : exigences d’opt-in, application de règle métier, type de configuration du canal et cas d’utilisation recommandés. [En savoir plus](../orchestrated/activities/channels.md#marketing-vs-transactional)

* Ajout dans la documentation sur l’**activité Branchement** d’une nouvelle section **Exemples** illustrant comment utiliser l’activité Branchement pour partager une audience sur deux branches d’e-mail parallèles (une marketing et une transactionnelle) dans une seule exécution de campagne. [En savoir plus](../orchestrated/activities/fork.md#fork-examples)

* Ajout dans la documentation sur l’**activité Création d’audience** d’un nouvel exemple montrant comment filtrer les profils en fonction d’un attribut de formule d’abonnement à l’aide du créateur de règles. [En savoir plus](../orchestrated/activities/build-audience.md#build-audience-examples)

* La page **Prise en main des campagnes orchestrées** présente le modèle de base **Création d’audience → Branchement → Canal A + Canal B** dans **Qu’est-ce qu’une campagne orchestrée ?**, avec des références croisées aux pages Activité Branchement et Messages marketing ou transactionnels. [En savoir plus](../orchestrated/gs-orchestrated-campaigns.md#gs-ms-campaign-inside)

* La page **Modifier le contenu d’un e-mail avec l’éditeur HTML avancé** a été déplacée de la section Gestion de contenu vers la section **E-mail** de la documentation. Cette page indique désormais que l’éditeur HTML avancé est disponible dans le Concepteur d’e-mail, pour les e-mails et pour les modèles de contenu d’e-mail. [En savoir plus](../email/email-expert-mode.md)

* Mise à jour de la documentation **Démarrage et surveillance des campagnes orchestrées** avec une nouvelle section détaillant la séquence d’exécution au moment de la publication interne, ainsi qu’un tableau du statut du cycle de vie de la campagne, une liste de contrôle de prépublication et un avertissement de confirmation d’envoi pour les campagnes non récurrentes. [En savoir plus](../orchestrated/start-monitor-campaigns.md#publication-sequence)

* Mise à jour de la documentation sur l’activité **Enregistrement de l’audience** avec une note indiquant que les activités Enregistrer l’audience s’exécutent toujours avant les activités de message au moment de la publication. [En savoir plus](../orchestrated/activities/save-audience.md)

* Trois nouvelles questions/réponses ont été ajoutées à la **FAQ sur les campagnes orchestrées** : ce qui se passe en interne au moment de la publication, une liste de contrôle de 7 raisons pour lesquelles les messages peuvent ne pas être envoyés après la publication et la différence entre la recherche d’instantané de profil et la résolution de profil en temps réel. [En savoir plus](../orchestrated/orchestrated-campaigns-faq.md)

* Ajout d’une nouvelle section **[Événements ignorés en raison d’une instance de parcours bloquée](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)** à la documentation de dépannage des parcours. Elle explique le motif de rejet de `maxInstanceStackEventsReached`, le moment où cela se produit et comment l’atténuer. Les mécanismes de sécurisation et les pages de liste des champs d’événement d’étape ont également été mis à jour.

* La documentation **Utilisation des fragments dans les politiques de décision** comprend désormais des notes sur les mécanismes de sécurisation pour le canal **E-mail** : **[!UICONTROL Simulation de contenu]** n’affiche pas les fragments d’expression de l’élément de décision, contrairement à **[!UICONTROL Envoi du BAT]** et les campagnes activées. La page indique également que les **[!UICONTROL fragments visuels]** ne peuvent pas être affectés à un élément de décision ; seuls les **fragments d’expression** sont pris en charge dans ce contexte. [En savoir plus](../experience-decisioning/fragments-decision-policies.md)

## Mars 2026 {#march-2026}

* La documentation sur la **prévisualisation des expériences basées sur du code avec Décisions pour les expériences** précise désormais que **[!UICONTROL Simulation de contenu]** est uniquement un aperçu du contenu. Les données contextuelles des requêtes Edge actives ne sont pas simulées dans l’aperçu de création. [En savoir plus](../code-based/test-code-based.md#preview-code-based)

* Mise à jour de la documentation **Utilisation des données Adobe Experience Platform** : les mécanismes de sécurisation n’indiquent plus que les recherches de jeux de données ne peuvent pas être liées, en cohérence avec le comportement actuel du produit. [En savoir plus](../data/lookup-aep-data.md)

* Mise à jour de la documentation sur l’activité **Mise à jour de profil** pour indiquer qu’il est désormais possible de mettre à jour jusqu’à cinq attributs de profil en une seule action. [En savoir plus](../building-journeys/update-profiles.md)

* Mise à jour de l’activité **Lecture d’audience** et de la documentation sur les **propriétés de parcours** afin de clarifier le cycle de vie du parcours de 91 jours pour les parcours récurrents permanents. La section de planning confirme désormais explicitement que les parcours récurrents sans date de fin restent actifs au-delà de 91 jours. La FAQ sur le délai d’expiration global a été enrichie afin de distinguer la durée de vie du profil de 91 jours de la fenêtre de reporting de 91 jours. [En savoir plus](../building-journeys/read-audience.md#schedule)

* Mise à jour de la documentation sur l’activité **Recherche de jeu de données** afin de clarifier le fait que la clé de recherche doit être configurée en mode avancé pour que la syntaxe `@datasetLookup{}` fonctionne dans les activités de condition en aval. Une section de dépannage a été ajoutée avec des conseils pour la résolution de l’erreur « Recherche de jeu de données introuvable ». [En savoir plus](../building-journeys/dataset-lookup.md#troubleshooting)

* Mise à jour de la documentation sur les **fonctions de date et d’heure** avec un nouvel exemple montrant comment formater une date et une heure à partir d’un attribut d’événement contextuel, y compris l’exigence `toDateTime()`, la syntaxe avec accents graves pour les ID d’événement numériques et un encadré sur l’erreur courante « entrée non concordante » de PQL. [En savoir plus](../personalization/functions/dates.md#format-date)

* Mise à jour de la documentation **Mécanismes de sécurisation et limites des campagnes orchestrées** et **Prise en main des connecteurs source** afin de clarifier que, pour la capture des données modifiées basée sur fichier, le champ `_change_request_type` est obligatoire et ses valeurs doivent être en minuscules `u` (upsert) ou `d` (delete), et non en majuscules. [En savoir plus](../orchestrated/guardrails.md)

* Mise à jour de la documentation **Ajout de liens et tracking des messages** avec des précisions sur la génération des identifiants de tracking (urlID) : un urlID unique n’est attribué que si l’URL et le libellé sont uniques. Pour suivre la même URL dans plusieurs e-mails (ou plusieurs fois dans un seul e-mail), les utilisateurs et utilisatrices doivent utiliser un libellé unique pour chaque URL similaire, sinon [!DNL Journey Optimizer] ne peut pas déterminer sur quel lien la personne a cliqué. [En savoir plus](../email/message-tracking.md#track-across-multiple-emails)

* Mise à jour de la documentation **Création de profils de test** avec une note importante sur les exigences en matière de descripteur d’identité : lorsqu’un jeu de données est supprimé puis recréé, le schéma doit conserver le bon descripteur d’identité sur le champ Identité principale. Sans cela, les profils ingérés ne seront pas marqués comme `testProfile = true`, même si l’ingestion a réussi. Une liste de contrôle de dépannage a été ajoutée. [En savoir plus](../audience/creating-test-profiles.md)

* Mise à jour de la documentation sur l’activité **Lecture d’audience** afin de clarifier le fait qu’une activité **Événement métier** constitue une exception à la règle selon laquelle la lecture d’audience doit être la première activité d’un parcours. Une note a également été ajoutée, présentant l’activité **Optimisation** comme une option avancée pour contrôler le ciblage des audiences. [En savoir plus](../building-journeys/read-audience.md)

* L’**envoi par vagues** dans les parcours est désormais disponible pour tout le monde. L’indicateur Disponibilité limitée a été supprimé de la documentation. [En savoir plus](../building-journeys/send-using-waves.md)

* Ajout à la documentation sur l’activité **Saut** d’une nouvelle section de stratégie de conception (**Sous-parcours digestes**) expliquant comment décomposer des flux complexes de bout en bout en sous-parcours plus petits et ciblés, connectés via l’activité Saut. [En savoir plus](../building-journeys/jump.md#jump-strategy)

* Mise à jour de la documentation **Balises** avec des précisions sur l’utilisation des catégories de balises comme alternative aux conventions de nommage complexes. Une nouvelle section explique comment configurer des catégories de balises pour une gestion de parcours évolutive. [En savoir plus](../building-journeys/tags.md)

* La documentation **À propos des sources de données** comprend désormais une nouvelle section qui aide les utilisateurs et utilisatrices à choisir entre trois stratégies d’accès aux données : accéder aux données externes par le biais d’actions personnalisées, utiliser un jeu de données non compatible avec les profils ou utiliser un jeu de données compatible avec les profils. Chaque option est accompagnée de ses limites et de recommandations de cas d’utilisation. [En savoir plus](../datasource/about-data-sources.md#data-access-strategy)

* Mise à jour de la documentation **Conception de notifications push** avec une note clarifiant le comportement des liens universels sur iOS : si l’URL de notification est enregistrée en tant que lien universel, l’application associée s’ouvre, quelle que soit l’action choisie pour l’URL web. Des instructions pour forcer l’ouverture du navigateur ont été ajoutées. [En savoir plus](../push/design-push.md)

* Une nouvelle page **Surveillance de vos modèles d’IA** est désormais disponible dans la documentation Prise de décision. Elle explique comment effectuer le suivi de l’intégrité, du statut de l’entraînement et des performances des modèles d’optimisation personnalisés directement dans [!DNL Journey Optimizer]. [En savoir plus](../experience-decisioning/ranking/ai-model-observability.md)

* L’**éditeur HTML avancé** (mode expert) pour les modèles d’e-mail est désormais en disponibilité limitée. La page de documentation est désormais accessible au public. Cette fonctionnalité vous permet d’afficher et de modifier la source HTML brute des modèles de contenu d’e-mail directement à partir du Concepteur d’e-mail. [En savoir plus](../email/email-expert-mode.md)

* Mise à jour de la documentation sur le **tracking des URL** et la **résolution des problèmes de parcours** afin d’expliquer le comportement de `context.system.source.actionId` dans les parcours fermés. Les parcours fermés ou non republiés peuvent produire des espaces réservés `{}` vides dans les URL de tracking. Des instructions ont été ajoutées pour résoudre le problème en republiant le parcours ou en supprimant le paramètre affecté. [En savoir plus](../email/url-tracking.md)

* Mise à jour de la documentation sur la **source de données Adobe Experience Platform** avec une note indiquant que seuls les schémas basés sur un profil individuel XDM sont pris en charge dans la configuration de la source de données. [En savoir plus](../datasource/adobe-experience-platform-data-source.md)

* Amélioration de la documentation sur les **mécanismes de sécurisation de la durée de vie des jeux de données** avec une nouvelle entrée de FAQ pour identifier clairement quels jeux de données ont une durée de vie. La durée de vie s’applique exclusivement aux jeux de données de série chronologique : les jeux de données de type enregistrement tels que les jeux de données d’entité, les jeux de données de classification et les référentiels d’objets de décision, n’ont pas de durée de vie et ne sont pas affectés par le déploiement du mécanisme de sécurisation. [En savoir plus](../data/datasets-ttl.md)

* Mise à jour de la documentation **Propriétés du parcours** et **Mettre en pause un parcours** afin de présenter les nouveaux champs de pause et de reprise désormais disponibles dans les détails techniques du parcours. Le bouton **Copier les détails techniques** comprend désormais `lastPausedAt`, `lastPausedBy`, `lastPausedById`, `lastResumedAt`, `lastResumedBy` et `lastResumedById`, en plus du bloc de `pausedJourneySettings` existant. Une nouvelle section a également été ajoutée à la page **Mettre en pause un parcours** pour expliquer comment consulter les dates et les heures de pause et de reprise directement dans les propriétés du parcours. [En savoir plus](../building-journeys/journey-properties.md)

## Février 2026 {#february-2026}

* Une nouvelle page est désormais disponible pour la gestion des décisions. Elle répertorie tous les opérateurs, assistants et fonctions pris en charge lors de la personnalisation du contenu des offres (représentations) avec l’éditeur de personnalisation. Utilisez cette liste pour éviter les erreurs d’exécution. Seules les fonctions documentées sont prises en charge pour la personnalisation de contenu dans Offer Decisioning. [En savoir plus](../offers/offer-library/personalization-editor-supported-functions.md)

* Mise à jour de la documentation **Création de politiques de décision** et **Utilisation des politiques de décision dans les messages** pour les e-mails : une note explique désormais que lorsqu’une même offre peut être sélectionnée par plusieurs politiques de décision dans le corps de l’e-mail, le moteur déduplique les offres (chaque emplacement reçoit une offre différente). Pour afficher la même offre à plusieurs emplacements (par exemple, en-tête et pied de page), utilisez **Réutilisation de la sortie de décision**. [En savoir plus](../experience-decisioning/create-decision-policy.md)

* Mise à jour de la page Éléments de décision avec des informations sur le canal push et la limitation des événements personnalisés. [En savoir plus](../experience-decisioning/items.md#capping)

* Mise à jour de la documentation **Recherche d’événement d’expérience dans les parcours** avec le calendrier d’obsolescence : à compter du 1er avril 2026, les organisations qui n’ont pas utilisé d’attributs d’événement d’expérience dans les expressions de parcours au cours des 90 derniers jours n’auront plus accès à cette fonctionnalité. La FAQ se concentre désormais sur la date de fin de vie et les personnes affectées, et la page Schéma d’événement d’expérience inclut un lien direct vers d’autres approches. [En savoir plus](../building-journeys/exp-event-lookup.md)

* La documentation **Prise de décision** a été mise à jour pour la **recherche de jeu de données** avec les données Adobe Experience Platform : le mécanisme de sécurisation des canaux pris en charge indique désormais que la recherche de jeu de données fonctionne pour tous les canaux pour lesquels la prise de décision est disponible (expérience basée sur du code, E-mail, Notification push, SMS et l’activité Décision de contenu dans les parcours). Les mentions de bêta publiques et de disponibilité limitée ont été supprimées des pages consacrées aux règles de décision, formules de classement et éléments de décision. [En savoir plus](../experience-decisioning/aep-data-exd.md)

* Mise à jour de la page sur l’intégration des systèmes externes avec des liens vers des sources de données et des actions personnalisées, et avec une précision indiquant que le proxy de sortie fournit une adresse IP statique pour les appels sortants depuis **Actions personnalisées** vers vos systèmes externes. [En savoir plus](../configuration/external-systems.md)

* Clarification de la documentation sur les tests à blanc des parcours : les attributs d’événement d’étape `inDryRun` et `dryRunID` précisent désormais qu’en mode Test à blanc, ils renvoient `true` ou un ID d’instance et `null` pour les parcours de test ou actifs. Les instructions pour exclure les événements d’étape Test à blanc dans les requêtes de reporting ont été adaptées. [En savoir plus](../building-journeys/journey-dry-run.md)

* **Notifications push web** est désormais disponible pour tout le monde. La documentation sur les notifications push a été restructurée et adaptée (prise en main, conception, envoi, création). [En savoir plus](../push/get-started-push.md)
...
* Mise à jour de la documentation sur l’API de migration Prise de décision. [En savoir plus](../experience-decisioning/decisioning-migration-api.md)

* L’activité **Décision de contenu** est désormais disponible pour tous. Mise à jour de la page de l’activité Décision de contenu avec une section sur les données de prise de décision disponibles dans les événements d’étape. [En savoir plus](../building-journeys/content-decision.md)
...
* Correction des informations sur les canaux pris en charge dans la documentation de l’assistant de création de campagne. Les pages Prise en main des canaux et FAQ sur les campagnes orchestrées ont été adaptées en conséquence. [En savoir plus](../campaigns/get-started-with-campaigns.md)

* Correction de la documentation sur les autorisations concernant les autorisations **Gérer des parcours** et **Approuver**. [En savoir plus](../administration/ootb-permissions.md)

* Mise à jour de la documentation sur les intégrations d’AEM (Adobe Experience Manager) avec la terminologie révisée (contenu dynamique AEM et fragments AEM). [En savoir plus](../integrations/aem-fragments.md)

* Ajout d’une nouvelle cause d’exclusion à la liste d’exclusion : **UnsubscribeLinkNotValid** (code d’erreur 050081). Cette exclusion est générée lorsque la longueur de l’objet List-Unsubscribe mailTo est supérieure à la limite RFC de 998 caractères. [En savoir plus](../reports/exclusion-list.md)

* Amélioration de la documentation de la fonction d’assistance formatDate avec une note indiquant que la fonction nécessite un type de champ date-heure (et non une chaîne) ainsi qu’avec plusieurs exemples : formatage d’un champ date-heure, conversion préalable d’une chaîne en date, date complète avec nom du jour, date dynamique à partir de l’heure système et format de jour de la semaine avec sortie en minuscules. [En savoir plus](../personalization/functions/dates.md#format-date)

* Amélioration de la documentation sur les e-mails de version texte avec des instructions d’utilisation complètes, y compris des critères de décision pour l’utilisation de texte brut personnalisé par rapport à la synchronisation automatique, des exemples pratiques avec des scénarios réels et une section FAQ avec les questions fréquentes. [En savoir plus](../email/text-version-email.md#when-to-use)

* La documentation sur les thèmes du Concepteur d’e-mail a été mise à jour avec des informations sur les limitations de prise en charge des polices web et l’importance des polices de secours. [En savoir plus](../email/apply-email-themes.md#themes-guardrails)

* Une limitation a été ajoutée à la documentation de l’assistant Métadonnées d’exécution pour préciser le fait que les métadonnées ne sont pas capturées pour les profils exclus de l’action. [En savoir plus](../personalization/functions/helpers.md#execution-metadata)

* La documentation sur les exemples d’implémentation basés sur le code a été mise à jour afin d’inclure le champ de jetons dans la propositionAction pour un tracking et une attribution précis dans la prise de décision. [En savoir plus](../code-based/code-based-implementation-samples.md#client-side-how)

* Une note a été ajoutée à la documentation sur le tracking des URL et la désinscription de la liste afin de préciser que l’ordre des paramètres de tracking d’URL ajoutés aux URL est aléatoire et ne peut pas être contrôlé. [En savoir plus](../email/url-tracking.md)

## Janvier 2026 {#january-2026}

* La documentation du tableau de bord d’utilisation des licences a été clarifié avec des conseils mis à jour sur les **profils engageables**, y compris des détails sur la définition et des conseils de dépannage. [En savoir plus](../audience/license-usage.md#what-is-engageable-profile)

* Une note a été ajoutée à la documentation sur les thèmes du Concepteur d’e-mail pour clarifier les limites de prise en charge des polices web. [En savoir plus](../email/apply-email-themes.md#themes-guardrails)
...
* Ajout d’une note à la section d’activité Condition du parcours pour documenter l’échec de l’évaluation de la condition pour les profils contenant plus de deux identités multi-appareils. [En savoir plus](../building-journeys/conditions.md)
...
* La page des actions de canal intégrée a été réorganisée. [En savoir plus](../building-journeys/journey-action.md)

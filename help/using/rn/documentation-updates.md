...
* La documentation **Exploiter les fragments dans les politiques de décision** comprend désormais des notes de mécanisme de sécurisation pour le canal **E-mail** : **[!UICONTROL Simuler du contenu]** n’affiche pas les fragments d’expression de l’élément de décision, contrairement à **[!UICONTROL Envoyer le BAT]** et les campagnes activées. La page indique également que les **[!UICONTROL fragments visuels]** ne peuvent pas être affectés à un élément de décision ; seuls les **fragments d’expression** sont pris en charge dans ce contexte. [En savoir plus](../experience-decisioning/fragments-decision-policies.md)

## Mars 2026 {#march-2026}

* La documentation relative à la **prévisualisation des expériences basées sur le code avec Experience Decisioning** précise désormais que la **[!UICONTROL Simuler du contenu]** est uniquement une prévisualisation de contenu. Les données contextuelles des requêtes Edge actives ne sont pas simulées dans l’aperçu de création. [En savoir plus](../code-based/test-code-based.md#preview-code-based)

* Mise à jour de la documentation sur la **Utilisation des données Adobe Experience Platform** : les mécanismes de sécurisation n’indiquent plus que les recherches de jeux de données ne peuvent pas être chaînées, ce qui reflète le comportement actuel du produit. [En savoir plus](../data/lookup-aep-data.md)

* La documentation de l’activité **Mettre à jour le profil** a été mise à jour afin de documenter la prise en charge de la mise à jour de jusqu’à cinq attributs de profil en une seule action. [En savoir plus](../building-journeys/update-profiles.md)

* L’activité **Lecture d’audience** et la documentation des **propriétés du Parcours** ont été mises à jour afin de clarifier le cycle de vie du parcours de 91 jours pour les parcours récurrents permanents. La section de planning confirme désormais explicitement que les parcours récurrents sans date de fin restent Actifs au-delà de 91 jours et la FAQ sur la temporisation globale a été étendue pour distinguer la durée de vie du profil de 91 jours de la fenêtre de création de rapports de 91 jours. [En savoir plus](../building-journeys/read-audience.md#schedule)

* Mise à jour de la documentation sur les activités de **recherche de jeu de données** afin de clarifier le fait que la clé de recherche doit être configurée en mode avancé pour que la syntaxe `@datasetLookup{}` fonctionne dans les activités de condition en aval. Une section de dépannage a été ajoutée avec des conseils sur la résolution de l’erreur « Recherche de jeu de données introuvable ». [En savoir plus](../building-journeys/dataset-lookup.md#troubleshooting)

* Mise à jour de la documentation relative aux **fonctions de date et d’heure** avec un nouvel exemple montrant comment formater une date et une heure à partir d’un attribut d’événement contextuel, y compris l’exigence de `toDateTime()`, la syntaxe de backtick pour les identifiants d’événement numériques et une légende d’erreur courante pour l’erreur « entrée non concordante » de PQL. [En savoir plus](../personalization/functions/dates.md#format-date)

* Les documents **Mécanismes de sécurisation et limitations des campagnes orchestrées** et **Prise en main des connecteurs source** ont été mis à jour afin de clarifier que pour la capture de données de modification basée sur un fichier, le champ `_change_request_type` est obligatoire et ses valeurs doivent être en minuscules `u` (upsert) ou `d` (delete), et non en majuscules. [En savoir plus](../orchestrated/guardrails.md)

* Mise à jour de la documentation **Ajouter des liens et tracker les messages** avec des conseils sur la génération des identifiants de tracking (urlID) : un urlID unique n&#39;est attribué que lorsque l&#39;URL et le libellé sont uniques. Pour suivre la même URL dans plusieurs e-mails (ou plusieurs fois dans un seul e-mail), les utilisateurs doivent utiliser un libellé unique pour chaque URL similaire ; autrement, [!DNL Journey Optimizer] ne pouvez pas déterminer sur quel lien l’utilisateur a cliqué. [En savoir plus](../email/message-tracking.md#track-across-multiple-emails)

* Mise à jour de la documentation **Créer des profils de test** avec une note importante sur les exigences en matière de descripteur d’identité : lorsqu’un jeu de données est supprimé et recréé, le schéma doit conserver le descripteur d’identité correct sur le champ Identité principale. Sans cela, les profils ingérés ne seront pas marqués comme `testProfile = true`, même si l’ingestion s’est terminée avec succès. Une liste de contrôle de dépannage a été ajoutée. [En savoir plus](../audience/creating-test-profiles.md)

* Mise à jour de la documentation sur l’activité **Lecture d’audience** afin de clarifier le fait qu’une activité **Événement métier** constitue une exception à la règle selon laquelle la lecture d’audience doit être la première activité d’un parcours. Une note a également été ajoutée, faisant référence à l’activité **Optimiser** en tant qu’alternative avancée pour contrôler le ciblage des audiences. [En savoir plus](../building-journeys/read-audience.md)

* **Envoyer par vagues** dans parcours est désormais disponible. L’indicateur Disponibilité limitée a été supprimé de la documentation. [En savoir plus](../building-journeys/send-using-waves.md)

* La documentation de l’activité **Saut** a été enrichie d’une nouvelle section de stratégie de conception (**Sous-parcours de la taille d’une bouchée**) expliquant comment diviser des flux complexes de bout en bout en sous-parcours plus petits et ciblés connectés via l’activité Saut. [En savoir plus](../building-journeys/jump.md#jump-strategy)

* Mise à jour de la documentation **Balises** avec des conseils sur l’utilisation des catégories de balises comme alternative aux conventions de nommage complexes. Une nouvelle section explique comment configurer des catégories de balises pour une gestion de parcours évolutive. [En savoir plus](../building-journeys/tags.md)

* La documentation **À propos des sources de données** comprend désormais une nouvelle section qui aide les utilisateurs à choisir entre trois stratégies d’accès aux données : accéder aux données externes par le biais d’actions personnalisées, utiliser un jeu de données non activé pour Profile ou utiliser un jeu de données activé pour Profile. Chaque option est décrite avec des compromis et des cas d’utilisation recommandés. [En savoir plus](../datasource/about-data-sources.md#data-access-strategy)

* Mise à jour de la documentation relative à la **conception des notifications push** avec une note clarifiant le comportement des liens universels sur iOS : si l&#39;URL de notification est enregistrée en tant que lien universel, l&#39;application associée s&#39;ouvrira, quelle que soit l&#39;action choisie pour l&#39;URL web. Des conseils ont été ajoutés pour forcer l’ouverture d’un navigateur. [En savoir plus](../push/design-push.md)
...
* Une nouvelle page est désormais disponible pour la gestion des décisions. Il répertorie tous les opérateurs, assistants et fonctions pris en charge lors de la personnalisation du contenu des offres (représentations) avec l’éditeur de personnalisation. Utilisez cette liste pour éviter les erreurs d’exécution. Seules les fonctions documentées sont prises en charge lors de la personnalisation de contenu dans Offer Decisioning. [En savoir plus](../offers/offer-library/personalization-editor-supported-functions.md)

* La documentation **Créer des politiques de décision** et **Utiliser des politiques de décision dans les messages** a été mise à jour pour les e-mails : une note explique désormais que lorsqu’une même offre peut être sélectionnée par plusieurs politiques de décision dans le corps de l’e-mail, le moteur déduplique les offres (chaque emplacement reçoit une offre différente). Pour afficher la même offre à plusieurs emplacements (par exemple, en-tête et pied de page), utilisez **Réutiliser la sortie de décision**. [En savoir plus](../experience-decisioning/create-decision-policy.md)

* La page Éléments de décision a été mise à jour avec des informations sur le canal push et la limitation des événements personnalisés. [En savoir plus](../experience-decisioning/items.md#capping)

* La documentation **Recherche d’événement d’expérience dans parcours** a été mise à jour avec le calendrier d’obsolescence : à compter du 1er avril 2026, les organisations qui n’ont pas utilisé d’attributs d’événement d’expérience dans les expressions de parcours au cours des 90 derniers jours n’auront plus accès à cette fonctionnalité. La FAQ se concentre désormais sur la date de suppression et sur les personnes affectées, et la page Schéma d’événement d’expérience a été alignée avec un lien direct vers d’autres approches. [En savoir plus](../building-journeys/exp-event-lookup.md)

* La documentation **Prise de décision** a été mise à jour pour la **recherche de jeu de données** avec les données Adobe Experience Platform : le mécanisme de sécurisation des canaux pris en charge indique désormais que la recherche de jeu de données fonctionne pour tous les canaux pour lesquels la prise de décision est disponible (expérience basée sur le code, e-mail, notification push, SMS et l’activité de décision de contenu dans les parcours). Les notes bêta publiques et de disponibilité limitée ont été supprimées des pages règles de décision, formules de classement et éléments de décision. [En savoir plus](../experience-decisioning/aep-data-exd.md)
...
* La documentation **Convertir des images en modèles de contenu** a été améliorée avec des mécanismes de sécurisation et des recommandations étendus, des cas d’utilisation courants et des conseils plus clairs pour convertir des conceptions d’image en modèles de contenu HTML modifiables. Il mentionne également le fait que vous pouvez désormais utiliser un thème comme entrée pour la conversion. [En savoir plus](../content-management/image-to-html.md)

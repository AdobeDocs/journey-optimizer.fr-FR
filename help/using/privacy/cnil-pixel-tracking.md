...
Le type d’e-mail est défini au niveau de la configuration du canal. Lors de la création d’un e-mail dans un parcours ou une campagne, les auteurs doivent sélectionner une configuration de canal dont le type d’e-mail correspond à la nature de la communication. Cette classification indique les contrôles de consentement appliqués avant la diffusion.
...
Au-delà du bouton (bascule) de suivi des ouvertures par message, Adobe Journey Optimizer Email Designer offre un contrôle granulaire sur les URL suivies. À l’aide du panneau Liens du Designer d’e-mail, les auteurs peuvent afficher toutes les URL suivies dans un message et définir le mode de suivi pour chaque lien individuellement.
...
Adobe Journey Optimizer gère le consentement via le schéma Consentement et préférences de Adobe Experience Platform (AEP). Les préférences de consentement sont stockées au niveau du profil et automatiquement appliquées lors du parcours et de l’exécution de la campagne.
...
**Centre de préférences via les pages de destination AJO**
...
Les rapports e-mail Adobe Journey Optimizer fournissent des mesures d’ouverture et de clic via les rapports dynamiques et les rapports Customer Journey Analytics. Lorsque le suivi **[!UICONTROL Ouvertures d’e-mails]** est désactivé pour un message, les données ouvertes ne sont pas collectées pour cette diffusion ; les rapports ne reflètent que les clics et d’autres signaux d’engagement.

## Références de documentation {#references}

Pour plus d’informations sur le suivi des e-mails et la gestion du consentement dans Adobe Journey Optimizer, consultez la documentation ci-dessous.

| Rubrique | Référence de documentation |
|-------|------------------------|
| Activer et désactiver le suivi des ouvertures | [Suivi des messages](../email/message-tracking.md) |
| Gestion du processus de désinscription aux e-mails | [Gestion des opt-outs pour les e-mails](../email/email-opt-out.md) |
| List-Unsubscribe (en-tête de l’e-mail) | [Configurer List-Unsubscribe](../email/list-unsubscribe.md) |
| Pages de destination du centre de préférences | [Cas d’utilisation de pages de destination](../landing-pages/lp-use-cases.md) |
| Gestion du consentement et de la désinscription | [Gérer les désinscriptions](opt-out.md) |
| Politiques de consentement | [Utiliser des politiques de consentement](../action/consent.md) |
| Configuration du canal e-mail | [Configurer les paramètres d’e-mail](../email/email-settings.md) |
| Liste de suppression | [Gérer la liste de suppression](../configuration/manage-suppression-list.md) |

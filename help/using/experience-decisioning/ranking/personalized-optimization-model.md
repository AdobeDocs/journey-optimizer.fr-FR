...
**Sur cette page :** comprenez comment le modèle d’optimisation personnalisée utilise le machine learning pour apprendre des données client, d’offre et contextuelles, y compris ses cas d’utilisation, ses composants de modèle d’ensemble, les exigences du jeu de données, les hypothèses et le comportement de démarrage à froid, afin que vous puissiez décider quand l’utiliser pour diffuser des offres personnalisées et optimiser vos KPI.

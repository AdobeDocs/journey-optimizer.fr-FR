...
### Identifier les adresses en quarantaine après une panne de FAI{#isp-outage-query}

En cas de panne du fournisseur d’accès à Internet (FAI), vous devez identifier les adresses email incorrectement retransmises en tant que bounces (mises en quarantaine) pour des domaines spécifiques, pendant une période donnée. Pour obtenir ces adresses, utilisez la requête suivante :

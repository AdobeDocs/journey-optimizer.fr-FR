...
title: Générer du contenu complet avec l’assistant IA
...
**Sur cette page :** découvrez comment utiliser l’assistant AI dans Adobe Journey Optimizer pour générer, affiner et finaliser des expériences de contenu textuel et d’image complètes pour vos canaux e-mail, web, de page de destination et de notification push.
...
Utilisez l’assistant IA dans Journey Optimizer pour générer des expériences de contenu complètes sur vos canaux e-mail, web, pages de destination et notifications push. L’assistant IA vous permet d’optimiser l’impact de vos diffusions en créant du contenu exhaustif qui résonne auprès de votre audience.

## Pour les canaux e-mail et web {#email-web-channels}

L’assistant IA peut produire des expériences de contenu complètes pour vos campagnes par e-mail, pages web et pages de destination en générant du texte et des images. Cette fonctionnalité robuste vous permet de créer du contenu attrayant et conforme à votre marque, qui résonne auprès de votre audience sur l’ensemble des points de contact numériques.

### Accès et configuration {#access-configure}

Avant de commencer à créer du contenu avec l’assistant IA, vous devez configurer votre campagne ou parcours et ouvrir l’éditeur de contenu. Suivez les étapes ci-dessous pour préparer votre espace de travail et accéder au panneau de l’assistant IA.

1. Créez et configurez votre campagne ou votre parcours :
   * **E-mail** : après avoir créé et configuré votre campagne, cliquez sur **[!UICONTROL Modifier le contenu]**. [En savoir plus](../campaigns/create-campaign.md)
...
1. Affinez le contenu en décrivant ce que vous souhaitez générer dans le champ **[!UICONTROL Prompt]**.
   ...
   ![Champ de prompt avec bouton Bibliothèque de prompts](assets/full-email-2.png){zoomable="yes"}
...
1. Sélectionnez vos **[!UICONTROL paramètres d’image]** :
   ...
   * **[!UICONTROL Modèle génératif]** : faites votre choix parmi les modèles intégrés disponibles, les modèles Firefly personnalisés formés sur les ressources de votre marque ou les fournisseurs de génération d’images tiers pour créer des images qui correspondent à vos besoins spécifiques et aux exigences de votre marque. [En savoir plus](generative-models.md). Pour **Gemini** avec **superpositions de texte** sur les images, consultez [Utiliser Gemini comme modèle génératif pour les images superposées de texte](generative-uc.md#generative-gemini).
...
Après avoir généré des variations de contenu, vous pouvez affiner les résultats pour vous assurer qu’ils répondent exactement à vos besoins. Examinez l’alignement sur la marque, ajustez le ton et le langage, puis préparez le contenu pour l’activation dans votre campagne ou votre parcours.
...
Pour utiliser l’assistant IA pour les notifications push, commencez par configurer votre campagne push et ouvrez l’éditeur de contenu. Les étapes ci-dessous vous guideront tout au long de la préparation de votre campagne et de l’accès aux outils de l’assistant IA.
...
1. Accédez au menu **[!UICONTROL Afficher l’assistant IA]**.
   ...
   ![Éditeur de notifications push avec le panneau Assistant IA ouvert](assets/push-genai-full-1.png){zoomable="yes"}
...
1. Affinez le contenu en décrivant ce que vous souhaitez générer dans le champ **[!UICONTROL Prompt]**.
   ...
   ![Assistant IA avec champ et options de prompt](assets/push-genai-full-2.png){zoomable="yes"}
...
1. Cliquez sur l’icône de pourcentage pour afficher votre **[!UICONTROL score d’alignement sur la marque]** et identifier tout décalage avec votre marque.
   ...
   ![Variations des notifications push générées avec score d’alignement sur la marque](assets/push-genai-full-4.png){zoomable="yes"}
...
1. Accédez à l’option **[!UICONTROL Affiner]** dans la fenêtre **[!UICONTROL Aperçu]** pour accéder à d’autres fonctionnalités de personnalisation :
   ...
   * **[!UICONTROL Modifier la stratégie de communication]** : modifiez l’approche des messages en fonction de vos objectifs, par exemple créer un sentiment d’urgence ou mettre en avant un aspect attractif et enthousiasmant.
     ...
     ![Options Affiner pour les notifications push](assets/push-genai-full-6.png){zoomable="yes"}

1. Ouvrez l’onglet **[!UICONTROL Alignement sur la marque]** pour voir comment votre contenu s’aligne sur vos [directives de marque](brands.md).

1. Cliquez sur **[!UICONTROL Sélectionner]** une fois que vous avez trouvé le contenu approprié.

1. Insérez des champs de personnalisation pour personnaliser le contenu de vos notifications push en fonction des données de profil. Ensuite, utilisez une méthode de simulation pour contrôler le rendu et vérifier les paramètres de personnalisation : cliquez sur **[!UICONTROL Simuler du contenu]** pour tester les variations de contenu avec des exemples de données d’entrée ou la génération automatique de l’IA, ou cliquez sur **[!UICONTROL Simuler du contenu]**, puis sélectionnez **[!UICONTROL Simuler du contenu (profils AEP)]** dans la liste déroulante pour obtenir un aperçu avec des profils de test. [En savoir plus](../content-management/preview-test.md)
...
Découvrez comment utiliser l’assistant IA dans Journey Optimizer afin de générer des expériences de contenu complètes.

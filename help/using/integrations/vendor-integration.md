...
1. Ajouter un en-tête d’authentification :

Autorisation : porteur &lt;CONTENTFUL_DELIVERY_TOKEN>
...
1. Suivez [Utilisation des intégrations](external-sources.md). Préférez la récupération d’un seul produit aux appels de catalogue en bloc, définissez l’authentification du porteur, collez l’exemple JSON, mappez les champs, testez, activez.
...
1. Suivez [Utilisation des intégrations](external-sources.md). Ajoutez des en-têtes `api_key` et `access_token` comme Contentstack l’exige, incluez le paramètre de requête `environment`, collez un exemple de code JSON, mappez des champs, testez et activez.
...
1. Suivez [Utilisation des intégrations](external-sources.md). Utilisez le modèle d’URL de diffusion publique que vos modules exposent, authentifiez selon les conseils de Magnolia (diffusion anonyme par rapport au jeton pour le contenu protégé), collez un exemple de JSON, mappez des champs, testez, activez.
...
1. Suivez [Utilisation des intégrations](external-sources.md). Définissez l’URL de base de votre cluster, ajoutez les en-têtes requis (`X-APP-ID`, `X-APP-TOKEN`), limitez les points d’entrée de liste avec des filtres ou des identifiants, collez un exemple de JSON, mappez des champs, testez, activez.

1. Dans Journey Optimizer, accédez à Configurations > Gérer, puis sélectionnez Créer une intégration.

1. Saisissez un nom d’intégration sans espaces.

1. Configurez le point d’entrée à l’aide des API REST/Loyalty. Exemple de modèle d’URL :

1. Par clé de vérification URL de base OpenAPI pour votre région
...
1. Configurez le point d’entrée à l’aide de l’API Antavo Enterprise. Exemple de modèle d’URL :

1. Par URL de base de pile Antavo documentée dans votre client
...
1. Suivez [Utilisation des intégrations](external-sources.md). Utilisez le point d’entrée d’intégration de fidélité approuvé par votre équipe, exécutez Salesforce OAuth, collez l’exemple JSON, mappez des champs, respectez les limites d’API composites, testez, activez.
...
1. Suivez [Utilisation des intégrations](external-sources.md). Configurez des en-têtes tels que `CAP-API-ACCESS-TOKEN` selon les besoins, collez l’exemple JSON, mappez des champs, testez, activez.

1. Dans Journey Optimizer, accédez à Configurations > Gérer, puis sélectionnez Créer une intégration.

1. Saisissez un nom d’intégration sans espaces.

1. Configurez le point d’entrée à l’aide des API Capillary. Exemple de modèle d’URL :

1. Guide d’intégration par capillaire pour votre région
...
1. Configurez le point d’entrée en utilisant l’URL de l’API des modèles Stensul (exemple de modèle) : `https://api.stensul.com/v1/templates/{template_id}`

1. Configurez l’authentification (clé API ou documentation OAuth par API Stensul).

1. Définissez les variables de chemin d’accès (par exemple, l’identifiant du modèle).
...
1. Suivez [Utilisation des intégrations](external-sources.md). Pointez sur l’hôte Marigold pour votre région, définissez l’authentification (l’exemple ci-dessous utilise `X-Api-Key` avec la clé et le secret), collez l’exemple JSON, mappez des champs, testez, activez.

1. Dans Journey Optimizer, accédez à Configurations > Gérer, puis sélectionnez Créer une intégration.

1. Saisissez un nom d’intégration sans espaces.

1. Configurez le point d’entrée à l’aide de l’API REST Marigold (point d’entrée selon votre guide d’intégration). Exemple de modèle d’URL :

1. Utilisez l’URL de base et le chemin d’accès fournis dans la documentation de votre API Marigold
...
1. Europe : {{customername}}.module.slgnt.eu
États-Unis : {{customername}}.module.slgnt.us
...
1. Configurez le point d’entrée à l’aide des API Target Recommendations/delivery (en fonction de la documentation Adobe pour votre modèle d’intégration). Exemple de modèle d’URL :

1. Consultez la documentation de l’API Recommendations Adobe Target pour votre cas d’utilisation
...
1. Suivez [Utilisation des intégrations](external-sources.md). Ciblez la ressource dont vous avez besoin (commandes ou expéditions), authentifiez-la par [API ShipStation](https://www.shipstation.com/docs/api/){target="_blank"}, collez l’exemple JSON, mappez des champs, testez, activez.
...
1. Suivez [Utilisation des intégrations](external-sources.md). Préférez les chemins de lecture étroits. Si vous utilisez l’exécution d’instruction **POST**, incluez le corps JSON dont l’API a besoin, collez un exemple de réponse de réussite pour le mappage, testez soigneusement la latence, puis activez.
...
1. Configurez le point d’entrée à l’aide de l’API Bynder v4 (exemple : modèle de liste des utilisateurs). Exemple de modèle d’URL :
...
| `stats` | `stats` | Variable | all | Non (désactivé) |
...
1. Suivez [Utilisation des intégrations](external-sources.md). Utilisez le schéma publié ou le chemin d’accès au centre de préférences de vos documents d’abonnement, renseignez OAuth si nécessaire, collez un exemple de code JSON, mappez des champs, testez, activez.

1. Dans Journey Optimizer, accédez à Configurations > Gérer, puis sélectionnez Créer une intégration.

1. Saisissez un nom d’intégration sans espaces.

1. Configurez le point d’entrée à l’aide de l’API OneTrust. Exemple de modèle d’URL :

1. Par URL de base du portail de développement OneTrust
...
1. Configurez le point d’entrée à l’aide de l’API Enregistrements / DAM Aprimo. Exemple de modèle d’URL :

1. Par URL de base d’API Aprimo et chemin d’accès aux ressources pour votre client
...
1. Suivez [Utilisation des intégrations](external-sources.md). Ne devinez pas les URL publiques. Utilisez la spécification Epsilon, collez l’exemple JSON, mappez les champs, testez, activez.

1. Dans Journey Optimizer, accédez à Configurations > Gérer, puis sélectionnez Créer une intégration.

1. Saisissez un nom d’intégration sans espaces.

1. Configurez le point d’entrée à l’aide de l’API Epsilon (conformément à votre spécification d’intégration). Exemple de modèle d’URL :

1. Fourni par Epsilon pour votre programme

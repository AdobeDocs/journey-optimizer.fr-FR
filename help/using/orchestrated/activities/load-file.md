...
**Sur cette page :** découvrez comment utiliser l’activité de gestion des données Chargement de fichier pour cibler une audience de campagne orchestrée à partir d’un fichier CSV ou TXT externe au moment de l’exécution, sans ingérer le fichier dans Adobe Experience Platform.
...
Pour obtenir des instructions sur l’attribution des autorisations, voir [Gérer les utilisateurs et les rôles](../../administration/permissions.md).
...
Avant de pouvoir ajouter une activité **[!UICONTROL Chargement de fichier]** à une campagne orchestrée et la connecter à une activité de message, un administrateur doit effectuer la configuration unique suivante.
...
Lors de la création de la dimension cible pour le ciblage basé sur des fichiers, veillez à :

* Définissez **[!UICONTROL Source]** sur **[!UICONTROL Fichier]**.
* Sélectionnez l’**[!UICONTROL Espace de noms d’identité]** qui correspond à la colonne d’identifiant dans vos fichiers, par exemple **[!UICONTROL E-mail]**.
* Saisissez le **[!UICONTROL chemin d’accès au champ d’identité]**. Utilisez le champ de fichier contenant l’identifiant, par exemple `email` si les fichiers chargés incluent une colonne `email`.

>[!CAUTION]
>
>Les valeurs de schéma et d’identité ne peuvent pas être modifiées une fois la dimension cible enregistrée. Vérifiez l’espace de noms d’identité et le chemin d’accès au champ d’identité avant d’enregistrer.

### Créer une configuration de canal pour la diffusion basée sur des fichiers {#file-channel-configuration}

Créez une configuration de canal dédiée qui utilise la dimension cible de type fichier. Cette configuration est sélectionnée dans l’activité de message qui suit l’activité **[!UICONTROL Chargement de fichier]** dans la zone de travail de votre campagne.

1. Accédez à **[!UICONTROL Administration]** > **[!UICONTROL Canaux]** > **[!UICONTROL Configurations de canal]** et créez une configuration.
...
>[!IMPORTANT]
>
>Les configurations de canal standard basées sur les profils ne fonctionnent pas avec une audience basée sur des fichiers. Utilisez une configuration de canal qui cible la dimension Type de fichier pour toute activité de message qui suit une activité **[!UICONTROL Chargement de fichier]**.
...
1. Dans la section **[!UICONTROL Fichier cible]**, sélectionnez le fichier CSV ou TXT contenant l’audience à cibler.
...
Une fois que **[!UICONTROL Chargement de fichier]** a résolu l’audience, connectez la transition sortante aux activités en aval. [Découvrez comment orchestrer des activités de campagne](../orchestrate-activities.md).

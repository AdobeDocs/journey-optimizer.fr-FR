...
**Sur cette page :** découvrez comment configurer le canal de notification push de l’application mobile dans Adobe Journey Optimizer, y compris les autorisations, les informations d’identification push, la configuration du canal et l’intégration de votre application au SDK mobile Adobe Experience Platform.
   ...
   * **Pour iOS**
     ...
     ![](assets/add-app-config-ios.png)
      ...
      1. Effectuez un glisser-déposer de votre fichier de clé d’authentification de notification push Apple .p8. Cette clé peut être obtenue à partir de la page **Certificats**, **Identifiants** et **Profils**.
         ...
         >[!NOTE]
         >
         > Seules les clés de notification push Apple .p8 sont prises en charge. Utilisez un autre compte de développeur Apple si vous avez atteint la limite de clés .p8.
...
>abstract="Lorsque vous gérez plusieurs applications mobiles, stockez chaque ID d’application sur le profil et utilisez une configuration des canaux de notification push unique. Ouvrez l’éditeur de personnalisation à côté du champ ID d’application pour sélectionner un attribut de profil. L’expression est évaluée au moment de l’envoi pour chaque destinataire. Assurez-vous que les informations d’identification de notification push existent pour chaque ID d’application que votre expression peut renvoyer."

...
1. Accédez au menu **[!UICONTROL Configuration du message]** / **[!UICONTROL Paramètres prédéfinis du message]**, puis cliquez sur **[!UICONTROL Créer un paramètre prédéfini de message]**.
...
1. Une fois le paramètre prédéfini de message créé, il s’affiche dans la liste avec l’état **[!UICONTROL Traitement]**.
   ...
   Vous pouvez suivre le processus de vérification en ouvrant le paramètre prédéfini de message à partir de la liste.

       -CAPTURE D&#39;ÉCRAN AFFICHANT LES DEUX VÉRIFICATIONS-
   
   !aucun paramètre prédéfini de traitement sur la scène + impossible d’envoyer un paramètre prédéfini

   * **xxxx** :
   * **Étape** d&#39;audit : les tests de délivrabilité sont effectués par l&#39;équipe de délivrabilité des Adobes. Cette étape peut prendre jusqu’à 10 jours si certaines étapes échouent au cours de l’audit. Les contrôles incluent :
...
1. Une fois les contrôles effectués, vous en serez averti ( ! !où ?). et le message prédéfini obtient l’état **[!UICONTROL Principal]**. Il est maintenant prêt à être utilisé pour diffuser des messages.

## Gestion et modification des paramètres prédéfinis de message

Tous vos paramètres prédéfinis de message s’affichent dans le menu **[!UICONTROL Configuration du message]** / **[!UICONTROL Paramètres prédéfinis de message]**. Des filtres sont disponibles pour vous aider à parcourir la liste (type de canal, utilisateur, état).
...
Pour modifier un paramètre prédéfini de message, vous devez d’abord le désactiver afin qu’il ne soit pas disponible pour créer de nouveaux messages (les messages publiés utilisant ce paramètre prédéfini ne seront pas affectés et continueront à fonctionner).

Vous devez ensuite duplicata le paramètre prédéfini de message pour créer une nouvelle version à utiliser pour créer de nouveaux messages :

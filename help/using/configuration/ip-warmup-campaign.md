...
**Sur cette page :** découvrez comment créer et activer des campagnes par e-mail dédiées au réchauffement des adresses IP afin qu’elles puissent être planifiées et utilisées dans un plan de réchauffement des adresses IP.
...
1. Suivez les étapes de création d’une campagne par e-mail, telles que définir les propriétés, l’[audience](../audience/about-audiences.md)<!--best practices for IP warmup in terms of audience?--> et le [contenu](../email/get-started-email-design.md#key-steps) de la campagne.
   ...
   >
   >Les audiences de chargement CSV ne sont pas prises en charge pour les campagnes de préchauffage d’adresses IP et entraîneront une erreur lors de l’activation de la campagne.
...
>[!NOTE]
>
>Lorsqu’une campagne active est utilisée dans un plan de préchauffage d’adresses IP, une fois que le plan est [marqué comme terminé](ip-warmup-execution.md#mark-as-completed), le [statut](../campaigns/manage-campaigns.md#statuses) de la campagne passe à **[!UICONTROL Arrêté]**.

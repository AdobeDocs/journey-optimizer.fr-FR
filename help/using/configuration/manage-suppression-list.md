...
Vous devez également ajouter un filtre sur le type de hard bounce, en fonction des détails de la panne. Ces détails sont fournis par le FAI, comme le code d’erreur exact renvoyé à l’expéditeur. Par exemple : `550 <email address> recipient rejected` ou `550 5.1.1 ‘email address’: user lookup success but no user record found`.

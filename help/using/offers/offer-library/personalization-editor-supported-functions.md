...
title: Fonctions prises en charge dans l’éditeur de personnalisation
description: Découvrez quelles fonctions d’éditeur de personnalisation sont prises en charge lors de la personnalisation du contenu de l’offre dans la gestion des décisions (Offer Decisioning).
...
# Fonctions prises en charge dans l’éditeur de personnalisation {#personalization-editor-supported-functions}

Dans la gestion des décisions, vous utilisez l’**éditeur de personnalisation** lorsque vous [ajoutez des représentations](add-representations.md) et personnalisez le **contenu de l’offre** (images, texte, liens dans vos offres).

Le serveur principal d’Offer Decisioning prend uniquement en charge un **sous-ensemble** des fonctions disponibles dans l’éditeur de personnalisation lors de la personnalisation de ce contenu. Cette page répertorie toutes les fonctions que vous pouvez utiliser en toute sécurité dans l’éditeur pour le contenu de l’offre. Développez chaque section pour afficher les opérateurs, les assistants et les fonctions pris en charge.

>[!NOTE]
>
>Cette liste de fonctions s&#39;applique **uniquement à la personnalisation du contenu de l&#39;offre** (représentations). Les règles de décision et les formules de classement utilisent différents éditeurs et ne se limitent pas à ce sous-ensemble.
...
>[!NOTE]
>
>Si vous utilisez une fonction qui ne figure pas dans la liste ci-dessus lors de la personnalisation du contenu d’une offre, l’expression peut échouer à l’exécution ou produire des résultats inattendus. Pour obtenir l’ensemble complet des fonctions disponibles dans [!DNL Journey Optimizer] personnalisation, voir [Liste des fonctions d’assistance](../../personalization/functions/functions.md). Seul le sous-ensemble documenté sur cette page est pris en charge pour la personnalisation du contenu dans Offer Decisioning.

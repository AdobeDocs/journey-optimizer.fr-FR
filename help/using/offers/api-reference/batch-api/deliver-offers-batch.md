...
# Diffusez des offres à l’aide de la fonction [!DNL Batch Decisioning] API
...
Pour préparer vos décisions, procédez comme suit :

* Pour exporter un résultat de décision, créez un jeu de données à l’aide du schéma &quot;Événements de décision ODE&quot;.
...
Toutes les demandes de prise de décision par lots nécessitent les en-têtes supplémentaires suivants :
...
| Propriété | Description | Exemple |
| -------- | ----------- | ------- |
| `xdm:segmentIds` | Identifiant unique du segment. Elle ne peut contenir qu’une seule valeur. | `609028e4-e66c-4776-b0d9-c782887e2273` |
...
| `xdm:propositionRequests` | wrapper contenant placementId et activityId |  |
...
Reportez-vous à la section [Documentation de la gestion des décisions](../../get-started/starting-offer-decisioning.md) pour une vue d&#39;ensemble des principaux concepts.
...
L’heure de bout en bout pour chaque décision de lot correspond à la durée entre le moment où la charge de travail est créée et le résultat de la décision disponible dans le jeu de données de sortie. La taille du segment dans le payload de la requête du POST est le facteur principal qui affecte le temps de décision du lot de bout en bout. Voici quelques observations pour différentes tailles de segments :

* taille du segment &lt;= 10K : 6 minutes
* taille du segment &lt;= 1M: 10 minutes
* taille du segment &lt;= 15M : 75 minutes

## Limites {#limitations}

Les restrictions suivantes s’appliquent à [!DNL Batch Decisioning] API :

* **Traitement par lot unique**: Actuellement, une seule tâche par lot peut être exécutée par jeu de données à la fois. Toutes les autres requêtes répondent avec HTTP 429 (Too many requests).
...
En suivant ce guide de l’API, vous avez vérifié l’état de la charge de travail et diffusé des offres à l’aide de la variable [!DNL Batch Decisioning] API. Pour plus d&#39;informations, consultez la [présentation de la gestion des décisions](../../get-started/starting-offer-decisioning.md).

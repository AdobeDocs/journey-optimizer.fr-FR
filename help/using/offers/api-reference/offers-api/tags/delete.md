...
Il peut parfois être nécessaire de supprimer (DELETE) une balise. Seules les balises que vous créez dans le conteneur client peuvent être supprimées. Pour ce faire, il vous suffit d’adresser une requête DELETE à l’événement [!DNL Offer Library] API utilisant le $id de la balise que vous souhaitez supprimer.

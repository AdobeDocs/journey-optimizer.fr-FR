...
description: Découvrez comment configurer votre environnement pour envoyer des messages mobiles avec Journey Optimizer avec un fournisseur personnalisé
...
**Sur cette page :** découvrez comment intégrer un fournisseur de messagerie personnalisé dans Adobe Journey Optimizer en créant des informations d’identification d’API, en choisissant une méthode d’authentification et en configurant des en-têtes, des payloads et des paramètres entrants pour envoyer des SMS et des messages RCS.
...
1. Ajoutez la **[!UICONTROL payload du fournisseur]** pour valider et personnaliser les payloads de votre requête.
   ...
   Pour les messages RCS, cette payload est ensuite utilisée lors de la [conception du contenu](create-mobile-message.md#sms-content).

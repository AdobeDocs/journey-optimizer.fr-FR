...
**Sur cette page :** découvrez comment créer des webhooks de SMS entrants et de commentaires dans Adobe Journey Optimizer pour capturer les réponses de consentement et les événements de diffusion pour les fournisseurs Sinch, Infobip et personnalisés.
...
>abstract="Vous pouvez configurer des webhooks pour capturer les réponses entrantes afin de gérer le consentement d’opt-in et d’opt-out, et pour recevoir des rapports de diffusion tels que des confirmations de lecture (le cas échéant)."


>[!CONTEXTUALHELP]
>id="ajo_admin_sms_webhook_flow_type"
>title="Choisir votre type de webhook"
>abstract="Lors de la configuration d’un webhook, choisissez **Entrant** pour capturer les réponses de consentement et les préférences d’utilisation ou **[!UICONTROL Commentaires]** pour effectuer le suivi des événements de diffusion et d’engagement pour la création de rapports et l’analyse."
...
1. Configurez vos paramètres webhook, comme décrit ci-dessous :
   ...
   * **[!UICONTROL Informations d’identification d’API]** : choisissez dans la liste déroulante les [informations d’identification d’API configurées précédemment](mobile-configuration-sinch.md).
...
➡️[En savoir plus sur la configuration des canaux](mobile-configuration-surface.md)
...
1. Configurez vos paramètres webhook, comme décrit ci-dessous :
   ...
   * **[!UICONTROL Informations d’identification d’API]** : choisissez dans la liste déroulante les [informations d’identification d’API configurées précédemment](mobile-configuration-infobip.md#api-credential).
...
1. Configurez vos paramètres webhook, comme décrit ci-dessous :
   ...
   * **[!UICONTROL Informations d’identification d’API]** : choisissez dans la liste déroulante les [informations d’identification d’API configurées précédemment](mobile-configuration-custom.md).
...
1. Créez une payload personnalisée correspondant au fichier JSON provenant du fournisseur. Notez que le seul format webhook pris en charge est JSON. Les données de formulaire pour les Webhooks ne sont pas prises en charge.
   ...
   * **InboundNumber** : numéro de téléphone utilisé pour cette configuration de webhook.
   ...
   >[!TIP]
   >
   > Ouvrez le **[!UICONTROL Guide de configuration]** pour obtenir un exemple de payload JSON et des conseils détaillés.

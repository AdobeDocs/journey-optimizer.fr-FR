...
À mesure que vos projets Adobe Journey Optimizer se développent, la recherche et l’organisation du contenu deviennent essentielles pour un travail efficace. Cette page vous explique comment localiser rapidement des parcours, des campagnes et des ressources à l’aide de la recherche universelle, comment filtrer les listes pour vous concentrer sur des éléments spécifiques et comment organiser votre travail à l’aide de balises et de catégories. Ces outils vous aident à parcourir de grands volumes de contenu, à maintenir la cohérence entre les équipes et à optimiser vos workflows quotidiens.

## Recherche {#unified-search}

Dans l’interface Adobe Journey Optimizer, utilisez la fonctionnalité de recherche [!DNL Adobe CX Enterprise] unifiée au centre de la barre supérieure pour rechercher des ressources, des parcours, des jeux de données et plus encore dans vos sandbox.
...
Dans la plupart des listes, utilisez la barre de recherche pour rechercher des éléments spécifiques et définir des critères de filtrage. Vous pouvez aussi trier les éléments de la liste en cliquant sur l’en-tête d’une colonne. Dans la vue des dossiers Campagnes, le tri par **[!UICONTROL priorité]** et **[!UICONTROL configuration des canaux]** est également pris en charge.
...
Dans les listes **Campagnes**, **Fragments** et **Modèles**, vous pouvez sélectionner plusieurs éléments à la fois à l’aide des cases à cocher et leur appliquer des opérations à partir d’une barre d’actions groupées qui s’affiche en bas de l’écran.
...
* **[!UICONTROL Modifier les balises]** : modifiez les balises attribuées aux éléments sélectionnés. [En savoir plus sur l’utilisation des balises →](#add-tags)
* **[!UICONTROL Gérer l’accès]** : appliquez les libellés d’accès aux éléments sélectionnés. [En savoir plus sur le contrôle d’accès au niveau de l’objet →](../administration/object-based-access.md)

...
L&#39;assistant `url` est utilisé pour effectuer le suivi des liens, raccourcir les URL et insérer des [liens profonds](../../email/deeplinks.md) dans le contenu de votre SMS.
...
|---|---|
| `originalUrl` | URL à raccourcir. |
| `type` | Type de lien. Utilisez `DEEPLINK` pour ouvrir un écran spécifique dans une application mobile. |
| `action` | Action de suivi. Utilisez `CLICK` pour effectuer le suivi des clics sur le lien. |
